    ...
    async (desde, hasta) => {
      ...
      try {
        ...
        if (ventas && Array.isArray(ventas)) {
          ...
          throw new Error(
            `Formato de datos incorrecto de la API. No se encontró 'detalles_ventas' o el formato es incorrecto. Respuesta completa: ${JSON.stringify(
              data
            )}`
          );
        }
        ...
        if (err.statusCode === 401) {
          ...
          errorMessage =
            ...
            (err.message || "Verifique la conexión.");
        }
          ...
          <tbody>
            {reporteData.map((item, index) => (
              <tr key={item.id_remito || index}>
                ...
                <td className="monto-columna">
                  ${" "}
                  {item.total_venta_item
                    ? item.total_venta_item.toFixed(2)
                    : "0.00"}
                </td>
